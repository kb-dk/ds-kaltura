...
MAIN_CLASS=dk.kb.kaltura.jobs.Retranscode
...
java $JAVA_OPTS -classpath "$CLASS_PATH" -Dlogback.configurationFile="$LOGBACK_CONF" -Ddk.kb
.applicationConfig="$SCRIPT_DIR/../conf/$APP_CONFIG" "$MAIN_CLASS" "-OutfileDir ."  "$@"
EXIT_CODE=$?
